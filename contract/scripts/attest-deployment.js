...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!flag?.startsWith("--") || !value) {
      throw new Error("Usage: node scripts/attest-deployment.js --rpc <url> --address <address>");
    }
    values[flag.slice(2)] = value;
  }
  if (!values.rpc || !values.address) {
    throw new Error("Both --rpc and --address are required");
  }
  return values;
}
