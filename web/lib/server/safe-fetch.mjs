...
function bounded(operation, signal) {
  return new Promise((resolve, reject) => {
    const abort = () => reject(new IntakeError("fetch_timeout", "Source time budget exceeded", false));
    if (signal.aborted) { abort(); return; }
    signal.addEventListener("abort", abort, { once: true });
    Promise.resolve().then(() => {
      if (signal.aborted) throw new IntakeError("fetch_timeout", "Source time budget exceeded", false);
      return operation();
    }).then(resolve, reject).finally(() => signal.removeEventListener("abort", abort));
  });
}

async function fetchSource(sourceUrl, options, signal) {
  ...
  for (let redirects = 0; ; redirects += 1) {
    ...
    const rawBody = await bounded(() => materializeBody(response.body, MAX_RESPONSE_BYTES), signal);
